...
void encriptarContra(char str[]) {
    for(int i = 0; (i < 100 && str[i] != '\0'); i++) {
        str[i] = str[i] + 3;
    }
    if (strcmp(str, "XjdqgdC")) {
        //printf("%s", str);
    }
    encriptar = true;
}
...
void menuPrincipal() {
    ...
    encriptarContra(contrasena);
    
    encriptarPrint("Bienvenido a la base de datos secreta \n");
    
    while (opcion != 4) {
        encriptarPrint("Seleccione una opcion\n");
        
        encriptarPrint(" 1. Agregar a un agente \n 2. Buscar a un agente \n 3. Ver a todos los agentes \n 4. Salir\n");
        scanf("%d", &opcion);
        ...
        switch (opcion) {
            ...
            case 2:
                flushstdin();
                encriptarPrint("Introduce el apellido del agente a borrar: ");
                fgets(apellido, sizeof(apellido), stdin);
                strtok(apellido, "\n");
            ...
            case 3:
                if (primerAgente != NULL) {
                    ...
                    encriptarPrint("Aun no hay agentes registrados\n");
                }
                break;
                
            default:
                encriptarPrint("Esa opcion no esta en el menu, prueba otra vez\n");
                break;
        }
    }
}
